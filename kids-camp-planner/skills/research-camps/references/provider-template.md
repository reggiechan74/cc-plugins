# [Camp Name]

## Basic Information
- **Provider**: [Organization name]
- **Location**: [Full address]
- **Website**: [URL]
- **Phone**: [Phone number]
- **Email**: [Contact email]

## Distance & Commute
- **Distance from home**: [X km]
- **Drive time**: [X minutes]
- **Transit accessible**: [Yes/No]
- **Parking**: [Available/Street/None]

## Programs Offered
### [Program Name 1]
- **Ages**: [X-Y]
- **Dates available**: [Weeks/dates]
- **Hours**: [Start-End]
- **Description**: [Brief description]

## Costs
| Item | Cost | Notes |
|------|------|-------|
| Base fee (weekly) | $[X] | |
| Before care | $[X]/week | [Hours] |
| After care | $[X]/week | [Hours] |
| Lunch program | $[X]/week | [Optional/Required] |
| Registration fee | $[X] | One-time |
| Materials fee | $[X] | [If applicable] |
| Sibling discount | [X]% | [Conditions] |
| Early bird discount | [Details] | [Deadline] |
| Multi-week discount | [Details] | [Minimum weeks] |

## Quality Indicators
- **Certifications**: [HIGH FIVE, OCA, etc.]
- **Staff ratios**: [Ratio for relevant age groups]
- **Staff training**: [Requirements]
- **Years operating**: [X years]
- **Reviews/reputation**: [Summary of reviews]

## Logistics
- **Registration status**: [Open/Closed/Waitlist]
- **Registration deadline**: [Date]
- **Cancellation policy**: [Summary]
- **What to bring**: [List]
- **Allergy policy**: [Nut-free, etc.]
- **Inclusion support**: [Special needs support available?]

## Suitability
- **Good fit for**: [Which children and why]
- **Concerns**: [Any issues or limitations]
- **Overall rating**: [Strong match / Good option / Backup / Not suitable]

## Notes
[Any additional observations, parent reviews, personal notes]

---
*Last updated: [Date]*
*Source: [How this info was gathered]*
